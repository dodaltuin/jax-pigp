...
class GPR:
    ...
    Observations in multiple function spaces accomodated through the
    use of an instance of InterdomainMeanFns to compute mean terms,
    and an instance of InterdomainKernelFns to compute covariance terms.
    """
    ...
    def get_common_values(
        ...
        err = train_data.ytrain - self.mean_fns.get_mh(params, train_data.Xtrain)
        ...
        Khh = self.kernel_fns.get_Khh(params, train_data.Xtrain)
        ...
        Lyyinv_err = linalg.solve_triangular(Lyy, err, lower=True)

        return err, Lyy, Lyyinv_err
    ...
    def posterior_predict(
        ...
        r"""
        Computes the mean and covariance of the posterior predictive distribution
        at test inputs Xs, i.e.

                p(us | Xs, train_data, params).

        Returns
        -------
            MeanAndCovariance: mean vector (mu) and covariance matrix (Sigma)
            of posterior distribution over test outputs us.
        """
